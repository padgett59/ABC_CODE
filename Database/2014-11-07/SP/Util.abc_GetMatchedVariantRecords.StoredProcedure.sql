/****** Object:  StoredProcedure [Util].[abc_GetMatchedVariantRecords]    Script Date: 07/31/2014 15:02:26 ******/
if object_id('Util.abc_GetMatchedVariantRecords', 'P') is null  --does the object already exist
begin
     exec('create procedure util.abc_GetMatchedVariantRecords as select 1') --create a simple stub procedure using dynamic sql
end
...
GO
ALTER PROCEDURE [Util].[abc_GetMatchedVariantRecords] 
...
SET @SQLString = @selectString +
	...
	and DiffType = ''V'' and Product = ' + convert(varchar(4),@product) + ' 
	 ';
